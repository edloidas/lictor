...
Lictor listens for GitHub App webhooks, verifies each delivery, and routes it to
a handler. It is a local process — you run it on your own machine and expose it
through a tunnel.
...
handler work is not bounded by that. The route verifies, decodes, forks the
handler with `Effect.forkDaemon`, and acks — so the ack means *accepted*, not
*finished*. Deliveries are durable on GitHub's side and replayable by id from
the App's **Advanced** tab, so nothing is lost that cannot be re-requested.
